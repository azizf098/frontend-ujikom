        ...
        <div class="overflow-hidden rounded-xl border shadow" style="margin-left: 0.5%;">
            <table class="min-w-full border-separate border-spacing-y-2 border-spacing-x-2">
                ...
                <tbody class="lg:border-gray-300">
                    <tr v-for="kategori in getKategori" :key="kategori.id">
                        ...
                        <button @click="deleteKategori(kategori.id)">
...
export default {
    ...
    methods: {
        ...mapActions('kategori', ['fetchKategori', 'addKategori']),
        async addData() {
            const data = {
                name: this.formKey.name,
            };
            try {
                const success = await this.$store.dispatch('kategori/addKategori', data);
                this.formKey.name = "";
        ...
        showEditPopup(kategori) {
            this.editFormData.id = kategori.id;
            this.editFormData.name = kategori.name;
